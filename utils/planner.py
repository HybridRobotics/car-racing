...
class OvertakePlanner:
    ...
    def get_local_path(self, xcurv_ego, time):
        ...
        num_of_horizon_planner = self.racing_game_param.num_of_horizon_planner
                ...
                if abs(s_other - xcurv_ego[4]) <= planning_prediction_factor*vehicles[name].xcurv[0] or abs(s_other + track.lap_length - xcurv_ego[4]) <= planning_prediction_factor*vehicles[name].xcurv[0] or abs(s_other - track.lap_length - xcurv_ego[4]) <= planning_prediction_factor*vehicles[name].xcurv[0]:
                    ...
                    if vehicles[name].no_dynamics:
                        obs_traj, _ = vehicles[name].get_trajectory_nsteps(
                            time, self.racing_game_param.timestep, num_of_horizon_planner+1)
                    else:
                        obs_traj, _ = vehicles[name].get_trajectory_nsteps(
                            num_of_horizon_planner+1)
                    obs_traj = obs_traj.T
        ...
        if s0 < 0:
            ey0 = f_optimal_ey(s0+track.lap_length)
        elif s0 < optimal_traj_xcurv[0, 4]:
            ey0 = optimal_traj_xcurv[0, 5]
        else:
            ey0 = f_optimal_ey(s0)
        # second point
        s1_l = s_veh - (planning_prediction_factor/3.0)*obs_traj[0, 0]
        ey1_l = track.width
        s1_r = s_veh - (planning_prediction_factor/3.0)*obs_traj[0, 0]
        ey1_r = -track.width
        # third point
        s2_l = s_veh + (planning_prediction_factor/3.0)*obs_traj[0, 0]
        ey2_l = track.width
        s2_r = s_veh + (planning_prediction_factor/3.0)*obs_traj[0, 0]
        ey2_r = -track.width
        ...
        opti_l = ca.Opti()
        eyvar_l = opti_l.variable(num_of_horizon_planner+1)
        opti_r = ca.Opti()
        eyvar_r = opti_r.variable(num_of_horizon_planner+1)
        cost_l = 0
        cost_r = 0
        bezier_xcurv_l = np.zeros((num_of_horizon_planner+1, 2))
        bezier_xcurv_r = np.zeros((num_of_horizon_planner+1, 2))
        bezier_xglob_l = np.zeros((num_of_horizon_planner+1, 2))
        bezier_xglob_r = np.zeros((num_of_horizon_planner+1, 2))
        for i in range(num_of_horizon_planner+1):
            t = i*(1.0/num_of_horizon_planner)
            bezier_xcurv_l[i, 0] = f_bezier_s(t, s0, s1_l, s2_l, s3)
            bezier_xcurv_l[i, 1] = f_bezier_ey(t, ey0, ey1_l, ey2_l, ey3)
            bezier_xcurv_r[i, 0] = f_bezier_s(t, s0, s1_r, s2_r, s3)
            bezier_xcurv_r[i, 1] = f_bezier_ey(t, ey0, ey1_r, ey2_r, ey3)
        f_bezier_l_ey = interp1d(bezier_xcurv_l[:, 0], bezier_xcurv_l[:, 1])
        f_bezier_r_ey = interp1d(bezier_xcurv_r[:, 0], bezier_xcurv_r[:, 1])
        # get the front and rear range of the obstacle vehicle
        vehicle_range_f = obs_traj[-1, 4]+1.0*vehicles["ego"].param.length
        vehicle_range_r = obs_traj[0, 4]-1.0*vehicles["ego"].param.length
        ...
        for i in range(num_of_horizon_planner+1):
            ...
            s_tmp = s_ego+(s_veh+planning_prediction_factor *
                           obs_traj[0, 0]-s_ego)*i/num_of_horizon_planner
            ...
            cost_l += alpha * \
                ((eyvar_l[i]-f_bezier_l_ey(s_tmp))**2)
            cost_r += alpha * \
                ((eyvar_r[i]-f_bezier_r_ey(s_tmp))**2)
            ...
            if (s_l < vehicle_range_r) or (s_l > vehicle_range_f):
                ...
                if i == 0 and xcurv_ego[5] <= (max_ey_obs+1.4*vehicles[agent_name].param.width):
                    pass
                else:
                    opti_l.subject_to(eyvar_l[i] > (
                        max_ey_obs+1.4*vehicles[agent_name].param.width))
            if (s_r < vehicle_range_r) or (s_r > vehicle_range_f):
                pass
            else:
                if i == 0 and xcurv_ego[5] >= (min_ey_obs-1.4*vehicles[agent_name].param.width):
                    pass
                else:
                    opti_r.subject_to(eyvar_r[i] <= (
                        min_ey_obs-1.4*vehicles[agent_name].param.width))
            ...
            cost_l += (1-alpha)*((eyvar_l[i] - f_optimal_ey(s_l))**2)
            cost_r += (1-alpha)*((eyvar_r[i] - f_optimal_ey(s_r))**2)
            # change rate
            if i >= 1:
            ...
            opti_l.subject_to(eyvar_l[i] <= 1.0*track.width)
            opti_l.subject_to(eyvar_l[i] >= -1.0*track.width)
            opti_r.subject_to(eyvar_r[i] >= -1.0*track.width)
            opti_r.subject_to(eyvar_r[i] <= 1.0*track.width)
        opti_l.subject_to(eyvar_l[0] == xcurv_ego[5])
        opti_l.subject_to(eyvar_l[num_of_horizon_planner] == ey3)
        opti_r.subject_to(eyvar_r[0] == xcurv_ego[5])
        opti_r.subject_to(eyvar_r[num_of_horizon_planner] == ey3)
        ...
        if cost_l < cost_r:
            ...
            print("right")
            direction_flag = -1
        target_traj = np.zeros((num_of_horizon_planner+1, 6))
        for i in range(num_of_horizon_planner+1):
            t = i*(1.0/num_of_horizon_planner)
        ...
        target_traj_xglob = np.zeros((num_of_horizon_planner+1, 6))
        for index in range(num_of_horizon_planner+1):
...
def f_bezier_s(t, s0, s1, s2, s3):
    f_bezier_s = s0*((1-t)**3) + 3*s1*t*((1-t)**2)+3*s2*(t**2)*(1-t)+s3*(t**3)
    return f_bezier_s


def f_bezier_ey(t, ey0, ey1, ey2, ey3):
    f_bezier_ey = ey0*((1-t)**3) + 3*ey1*t*((1-t)**2) + \
