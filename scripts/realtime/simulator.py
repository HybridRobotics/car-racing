...
from utils import vehicle_dynamics, racing_env
from car_racing.msg import VehicleControl, VehicleState, VehicleStateGlob, VehicleStateCurv, NumVehicle, TrackInfo, VehicleList, OptimalTraj
...
def start_simulator(track_layout):
    ...
    sim.__pub_veh_list = rospy.Publisher(
        'vehicle_list', VehicleList, queue_size=10)
    sim.__pub_veh_num = rospy.Publisher(
        'vehicle_num', NumVehicle, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        for name in sim.vehicles:
            ...
            sim.vehicles[name].__pub_state.publish(
                sim.vehicles[name].msg_state)
