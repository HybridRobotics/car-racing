    ...
    for alpha in alpha_list:
        if args["simulation"]:
            ...
            lmpc_controller, time_lmpc = set_up_lmpc(
                timestep, track, lap_number, alpha, opti_traj_xcurv, opti_traj_xglob)
            # define a simulator
            ...
            if args["sim_replay"]:
                with open("data/simulator/racing_game.obj", "rb") as handle:
                    ...
                    for index in range(0, num):
                        ...
                        veh_list[index].set_state_curvilinear_func(
                            t_symbol, car_ini_xcurv[0] * t_symbol + car_ini_xcurv[4], car_ini_xcurv[5] + 0.0 * t_symbol)
                ...
                for index in range(0, num_veh):
                    veh_name = "car"+str(index+1)
                    veh_list[index].set_state_curvilinear_func(t_symbol, 0.1*random.randint(
                ...
                for index in range(0, num_veh):
                    veh_name = "car"+str(index+1)
                    veh_list[index].set_state_curvilinear_func(
                        t_symbol, (0.3+index*0.3) * t_symbol + 3+index*2, -0.3+index*0.2 + 0.0 * t_symbol)
                    simulator.add_vehicle(veh_list[index])
            if args["direct_lmpc"]:
                pass
            else:
...
def set_up_other_vehicles(track, num_veh):
    ...
    for index in range(0, num_veh):
        veh_name = "car"+str(index+1)
        veh_list.append(offboard.NoDynamicsModel(
