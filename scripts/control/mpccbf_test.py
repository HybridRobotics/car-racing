...
def racing(args):
    ...
    if args["simulation"]:
        ...
        ego.start_logging()
        ego.set_ctrl_policy(offboard.MpcCbfRacingOffboard(mpc_cbf_param, ego.system_param))
        ego.ctrl_policy.set_timestep(0.1)
        ego.set_track(track)
        ego.ctrl_policy.set_track(track)
        # setup surrounding cars
        t_symbol = sp.symbols("t")
        car1 = offboard.NoDynamicsModelOffboard(name="car1", param=base.CarParam(edgecolor="orange"))
        car1.set_track(track)
        car1.set_state_curvilinear_func(t_symbol, 0.2 * t_symbol + 4.0, 0.1 + 0.0 * t_symbol)
        car1.start_logging()
        car2 = offboard.NoDynamicsModelOffboard(name="car2", param=base.CarParam(edgecolor="orange"))
